...
public class NN implements Serializable{
	...
	public NN(int inputDimension, int numOfHiddenNodes, int outputDimension, int mode, double[] data, int sex, int iteration, double[][] itoh, double[] htoo ){
...
// SINCE we are having object for every file we are initializing the weights in the constructor
		...
		if(iteration == 0 && sex == 0){
			...
			wtsInputToHidden = itoh;
			wtsHiddenToOutput = htoo;
		}
	...
	double test( double[][] weightsItoH, double[] weightsHtoO, int gender ){
		...
		if(gender == 1){
			if (finaloutput > 0.75)
			System.out.println("Testdata shows that this image is of MALE " );
			else 
			System.out.println("Testdata shows that this image is of FEMALE " );
		 }
		 else if(gender == 0){
			if (finaloutput > 0.62)
			System.out.println("Testdata shows that this image is of MALE " );
